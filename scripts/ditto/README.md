...
- `data/wdc/wdcproducts80cc20rnd000un_train_*.json.gz`
- `data/wdc/wdcproducts80cc20rnd000un_valid_*.json.gz`

## 1) Build pseudo-labeled datasets (train and val separately)

```bash
python scripts/ditto/build_wdc_pseudolabels.py \
  --source-json-gz data/wdc/wdcproducts80cc20rnd000un_train_medium.json.gz \
...
```bash
python scripts/ditto/build_wdc_pseudolabels.py \
  --source-json-gz data/wdc/wdcproducts80cc20rnd000un_valid_medium.json.gz \
  --multi-agent-run-dir output/multi_agent_single_batch/run_YYYYMMDD_HHMMSS \
...
```bash
python scripts/ditto/convert_wdc_to_ditto.py \
...
python scripts/ditto/train.py \
  ...
  --test-json-gz data/wdc/wdcproducts80cc20rnd100un_gs.json.gz \
...
```bash
SPLIT_SIZE=medium bash scripts/ditto/run_existing_wdc.sh
```

## 4) Train (DDP / multi-GPU)

```bash
torchrun --standalone --nproc_per_node 2 scripts/ditto/train.py \
  --ddp \
  --train-json-gz output/pseudolabels/train_pseudo.json.gz \
  --val-json-gz output/pseudolabels/val_pseudo.json.gz \
  --test-json-gz data/wdc/wdcproducts80cc20rnd100un_gs.json.gz \
