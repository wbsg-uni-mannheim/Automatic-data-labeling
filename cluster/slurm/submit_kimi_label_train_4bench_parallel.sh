...
#   export LABEL_OUTPUT_ROOT=output/ditto_labeling/kimi
#   export DITTO_OUTPUT_ROOT=output/training_from_generated_labels/kimi
...
LABEL_OUTPUT_ROOT="${LABEL_OUTPUT_ROOT:-output/ditto_labeling/kimi}"
DITTO_OUTPUT_ROOT="${DITTO_OUTPUT_ROOT:-output/training_from_generated_labels/kimi}"
...
RUN_NAME_PREFIX_TRAIN="${RUN_NAME_PREFIX_TRAIN:-generated_ditto_kimi_k26}"
DRY_RUN="${DRY_RUN:-0}"

LABEL_SBATCH="cluster/slurm/run_three_phase_labeling_kimi_k26.sbatch"
...
submit_pair() {
  ...
  if [ "${DRY_RUN}" = "1" ]; then
    ...
    LABEL_JOB_ID="$(
      ...
      sbatch \
        --parsable \
        --job-name="active-${SAFE_BENCH}-kimi-k26" \
        --time="${LABEL_TIME}" \
        --output="logs/active-${SAFE_BENCH}-kimi-k26-%j.out" \
        --error="logs/active-${SAFE_BENCH}-kimi-k26-%j.err" \
        --export=ALL \
        "${LABEL_SBATCH}"
    )"
  ...
  if [ "${DRY_RUN}" = "1" ]; then
    ...
    TRAIN_JOB_ID="$(
      ...
      METHOD_NAME="kimi_k26_three_phase_v2" \
      sbatch \
        --parsable \
        --dependency="afterok:${LABEL_JOB_ID}" \
        --kill-on-invalid-dep=yes \
        --job-name="ditto-${SAFE_BENCH}-kimi-k26-3runs" \
        --time="${TRAIN_TIME}" \
        --output="logs/ditto-${SAFE_BENCH}-kimi-k26-3runs-%j.out" \
        --error="logs/ditto-${SAFE_BENCH}-kimi-k26-3runs-%j.err" \
        --export=ALL \
        "${TRAIN_SBATCH}"
    )"
